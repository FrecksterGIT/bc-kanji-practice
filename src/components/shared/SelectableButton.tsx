...
const SelectableButton: FC<SelectableButtonProps> = ({ term, selected, valid, onClick, type }) => {
  ...
  return (
    ...
    >
      {!valid && <NoAnswer />}
      {valid && <CorrectAnswer />}
      <div className={`text-xl text-nowrap text-left rounded-lg px-2 py-1 ${getButtonClass()}`}>
        {term}
      </div>
    </button>
  );
